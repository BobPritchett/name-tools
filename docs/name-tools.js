...
var TERMINAL_ORGANIZATION_NOUNS = [
  "Company",
  "Co.",
  "Laboratory",
  "Laboratories",
...
var TERMINAL_ORG_NOUN_RE = new RegExp(
  `\\b(${TERMINAL_ORGANIZATION_NOUNS.map((term) => term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|")})\\.?$`,
  "i"
);
...
function matchTerminalOrganizationNoun(text) {
...
function detectOrganization(normalized, raw) {
  ...
  let legalSuffixRaw;
  let legalForm;
  let organizationTermRaw;
  let organizationTermKind;
  let aka;
  ...
  const isOrg = reasons.some(
    (r) => r === "ORG_LEGAL_SUFFIX" || r === "ORG_INSTITUTION_PHRASE" || r === "ORG_TERMINAL_NOUN" || r === "ORG_DBA"
  );
  if (!isOrg) {
...
function looksLikeUnknownPostNominalChunk(value) {
...
function makeWarning(code, message, token, index) {
  return { code, message, token, index };
}
function detailsFromWarnings(warnings) {
...
function collectGivenNameEvidence(tokens, raw, options, candidateIndices) {
  if (!options.givenNameEvidence)
    return void 0;
  const matches = [];
...
function evidenceAt(evidence, index) {
  return evidence?.find((e) => e.index === index);
}
function buildPersonTokens(parts) {
  const tokens = [];
  if (parts.honorific)
    tokens.push({ type: "prefix", value: parts.honorific });
  if (parts.given)
    tokens.push({ type: "given", value: parts.given });
  ...
  if (parts.suffix)
    tokens.push({ type: "suffix", value: parts.suffix });
  if (parts.nickname)
    tokens.push({ type: "nickname", value: parts.nickname });
  return tokens;
}
function familyPartsFrom(family) {
  const parts = family?.split(/\s+/).filter(Boolean);
  return parts && parts.length > 0 ? parts : void 0;
}
function findParticleSurnameStart(tokens) {
  let start = null;
...
function tryParseReversed(normalized, options = {}) {
  let text = normalized;
  let nickname;
...
function parseStandardFormat(normalized, options = {}) {
  const reasons = [];
  let confidence = 0.5;
  ...
  if (tokens.length === 1) {
    ...
    if (particleStart !== null) {
      ...
      const allInteriorCheckedNotFound = interiorEvidence.length > 0 && interiorEvidence.every((e) => e?.found === false);
      if (allInteriorCheckedNotFound) {
        family = tokens.slice(1).join(" ");
        evidenceUsed = true;
        parseConfidence = 0.65;
      } else {
        additionalGiven = interiorTokens.join(" ") || void 0;
        middle = additionalGiven;
...
function detectPerson(normalized, options = {}) {
...
function finalizeParsedName(result) {
...
function extractSuffixes(text, result) {
  ...
  const looksLikeUnknownPostNominalChunk2 = (value) => {
    const v = value.trim().replace(/^[,;:\s]+/, "").replace(/[,;:\s]+$/, "");
    if (!v)
      return false;
    if (v.length > 18)
  ...
  while (parts.length > 1) {
    ...
    if (looksLikeKnownOrHeuristicSuffix(firstWordOfLast) || looksLikeKnownOrHeuristicSuffix(lastPart) || looksLikeUnknownPostNominalChunk2(lastPart) || /queen|king|consort/i.test(lastPart)) {
...
function entityToLegacy(entity) {
  ...
  if (person.additionalGiven)
    result.additionalGiven = person.additionalGiven;
  if (person.family)
    result.last = person.family;
  if (person.familyParts)
    result.familyParts = person.familyParts;
  if (person.suffix)
    result.suffix = person.suffix;
  if (person.nickname)
    result.nickname = person.nickname;
  if (person.tokens)
    result.tokens = person.tokens;
...
function renderMiddle(parsed, middleMode, o, t) {
  ...
  if (!middle)
    return void 0;
  if (parsed.ambiguousMiddleOrFamily)
    return middle;
  if (middleMode === "none") {
    return void 0;
  }
...
function renderSingle(parsed, o) {
  ...
  if (o.order === "family-given") {
    if (o.particleFiling === "ignoreLeading" && safeParsed.familyParticle && safeParsed.familyParts && safeParsed.familyParts.length > 0) {
      ...
      givenLikeText = givenLikeText ? `${givenLikeText}${boundarySpace("space", o, t)}${safeParsed.familyParticle}` : safeParsed.familyParticle;
    }
    const pieces = [];
    if (lastText)
      pieces.push(lastText);
...
function createGivenNameEvidenceProvider(db) {
  return (token) => {
